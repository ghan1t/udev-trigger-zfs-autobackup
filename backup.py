...
# Backup function
def import_decrypt_backup_export(device_label: str, config: AppConfig, logger: logging.Logger) -> None:
    ...
    try:
        logger.info(f"Importing pool {device_label}")
        ...
        logger.info(f"Exporting {device_label}")
...
def decrypt_and_backup(device_label: str, pool_config: PoolConfig, config: AppConfig, logger: logging.Logger) -> Optional[str]:
    if pool_config.passphrase:
        logger.info(f"Decrypting pool {device_label}")
        result = decrypt_pool(device_label, pool_config.passphrase, logger)
        if result.returncode != 0:
            mail_error(f"Failed to decrypt pool. Backup not yet run.\n\nError:\n{result.stderr}", config.email, logger)
            return None

    logger.info(f"Starting ZFS-Autobackup for pool {device_label} with parameters:\nzfs-autobackup " + " ".join(pool_config.autobackup_parameters))
    ...
    logger.info(f"Setting pool {device_label} to read-only")
...
def run_zfs_autobackup(args: list[str], logger: logging.Logger) -> tuple[bool, str, str]:
    """
...
def import_pool(pool: str, logger: logging.Logger) -> subprocess.CompletedProcess:
    return run_command(logger, ["zpool", "import", pool, "-N"])


def export_pool(pool: str, logger: logging.Logger) -> subprocess.CompletedProcess:
    return run_command(logger, ["zpool", "export", pool])


def decrypt_pool(pool: str, passphrase: str, logger: logging.Logger) -> subprocess.CompletedProcess:
    return run_command(logger, ["zfs", "load-key", pool], input_data=passphrase)


def set_pool_readonly(pool: str, logger: logging.Logger) -> subprocess.CompletedProcess:
    return run_command(logger, ["zfs", "set", "readonly=on", pool])


def run_command(logger: logging.Logger, command: list[str], input_data: Optional[str] = None) -> subprocess.CompletedProcess:
